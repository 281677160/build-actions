...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.2.8'                                    # IPv4 地址(openwrt后台地址)
...
sed -i 's/PATCHVER:=5.15/PATCHVER:=5.4.162/g' target/linux/x86/Makefile                               # x86机型,默认内核5.10，修改内核为5.4（去掉sed前面的#生效）
#sed -i 's/KERNEL_PATCHVER:=5.15/KERNEL_PATCHVER:=5.4.162/g' ./target/linux/x86/Makefile
