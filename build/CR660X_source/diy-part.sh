...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.2.239'                      # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                   # IPv4 子网掩码
uci set network.lan.gateway='192.168.2.2'                    # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
...
sed -i "s/OpenWrt /wh compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ
