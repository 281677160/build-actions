...
2023年5月13号
 ...
 1、重新整理了所有源码的插件包，因技术有限，gl-ax1800源码增加不了docker，还有个别源码不能编译ssrplus和passwall的NaiveProxy，个别源码的vssr可以编译成功，但是不能用，还有没有其他插件也存在此问题，等你们测试了
 ...
 5、自定义设置里面的（export Enable_IPV6_function="0"）和（export Enable_IPV4_function="0"）的设置问题，这两个只能二选一的，你要编译什么类型固件就选什么，编译IPV4固件的时候，因为不能完全的把IPV6功能全部去掉，如果全部去掉的话，个别插件是不能选择，也不能编译的，那就只能打开（export Enable_IPV4_function="1"），编译好固件后，安装固件完毕，然后把不能去掉的IPV4选项全部给关闭了。如果你要编译IPV6功能的固件，就打开（export Enable_IPV6_function="1"），就会在源码的时候就帮你把IPV6需要的选项都勾选上，编译固件后就是IPV6功能固件了。
