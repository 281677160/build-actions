...
- 出现这样情况很有可能是连接SSH进去配置过固件的原因，如果有这情况出现，停了编译后也会有一份配置文件的，你下载配置文件，然后把内容覆盖到你编译的对应源码的.config里面，不要进SSH再次编译试试，也可以直接用我的本地一键提取.config配置来提取到配置文件覆盖到你想编译的源码的.config里面
